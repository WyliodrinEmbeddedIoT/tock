...
impl<'a> Spi<'a> {
    pub const fn new_spi0(clock: clocks::Clocks) -> Self {
        Self {
            registers: SPI0_BASE,
            clock,
            clocks: clocks::Clock::Peripheral,
    ...
    pub const fn new_spi1(clock: clocks::Clocks) -> Self {
        Self {
            registers: SPI1_BASE,
            clock,
            clocks: clocks::Clock::Peripheral,
    ...
    pub fn handle_interrupt(&self) {
        if self.registers.sspsr.is_set(SSPSR::TFE) {
            ...
            if self.tx_buffer.is_some() && self.tx_position.get() < self.len.get() {
                ...
                self.registers.sspimsc.modify(SSPIMSC::TXIM::CLEAR); // interrupt
                self.transfers
                    .set(self.transfers.get() & !SPI_WRITE_IN_PROGRESS);
            }
        }

        // if self.registers.sspsr.is_set(SSPSR::RNE) {
    ...
    fn read_write_bytes(
        ...
        if self.transfers.get() == 0 {
            self.registers.sspimsc.modify(SSPIMSC::RXIM::CLEAR); // interrupt
            self.active_slave.map(|p| {
                p.clear();
            });
