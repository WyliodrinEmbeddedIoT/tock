...
pub struct NinaW102<'a, S: SpiMaster<'a>, P: Pin, A: Alarm<'a>> {
    ...
    scanner_client: OptionalCell<&'a dyn ScannerClient>,
    station_client: OptionalCell<&'a dyn StationClient>,
    second_time: Cell<u8>,
}
    ...
    fn send_command<'b>(&self, command: Command, params: &'b [&'b [u8]]) -> Result<(), ErrorCode> {
        ...
        self.write_buffer
            ...
            .map_or(Err(ErrorCode::NOMEM), |buffer| {
                ...
                self.spi
                    .read_write_bytes(buffer, self.read_buffer.take(), position + 1)
